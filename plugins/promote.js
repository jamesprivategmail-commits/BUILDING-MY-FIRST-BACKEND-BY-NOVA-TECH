...
export default {
    command: 'promote',
    description: 'Reply to a member to make them admin (admin only)',
    async handler(sock, msg, args, { chatId }) {
        ...
        if (!target) {
            await sock.sendMessage(chatId, { text: '❌ Reply to a member or use .promote <number>' }, { quoted: msg });
            return;
        }
        await sock.groupParticipantsUpdate(chatId, [target], 'promote');
        await sock.sendMessage(chatId, { text: `✅ Promoted ${target.split('@')[0]} to admin.` });
    },
};